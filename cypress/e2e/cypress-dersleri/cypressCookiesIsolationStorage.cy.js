...
describe('Cypress Clear Cookies', () => {
    it('Cypress Cookies', () => {
        ...
        cy.getCookies().should('have.have.length', 4)

        cy.clearCookies()

        cy.getCookies().should('have.have.length', 0)
...
describe.only('Cypress Cookie Ekleme', () => {
...
describe('Cypress Test Isolation - 2', {testIsolation: false}, () => {
    ...
    it('Test Isolation icin Ornek 2', () => {
        ...
        cy.get('#btn-display-inputs').should('be.visible').click()
        cy.get('#output-number').should('be.visible').and('contain', '123')
        });
});
...
describe('Cypress Local Storage', () => {
    it('Cypress Local Storage Ornek', () => {
        ...
        cy.wait(2000).then(() => {
            localStorage.removeItem('clarusway', 'test')
        ...
        cy.wait(1000).then(() => {
            ...
            cy.log(localStorage.length) //local storage 0 olarak yazdiracak
        })
    });
});